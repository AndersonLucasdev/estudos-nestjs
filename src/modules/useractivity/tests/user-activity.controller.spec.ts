...
describe('UserActivityController', () => {
  ...
  it('should get user activities by type', async () => {
    ...
    jest.spyOn(service, 'getUserActivitiesByType').mockResolvedValue(mockActivities);

    const result = await controller.getUserActivitiesByType(userId, activityType);

    expect(result).toEqual(mockActivities);
    expect(service.getUserActivitiesByType).toHaveBeenCalledWith(userId, activityType);
  });
  ...
  it('should count user activities by type', async () => {
    ...
    const activityType = 'login';
    const mockCount = 5;
    jest.spyOn(service, 'countUserActivitiesByType').mockResolvedValue(mockCount);

    const result = await controller.countUserActivitiesByType(userId, activityType);

    expect(result).toEqual(mockCount);
    expect(service.countUserActivitiesByType).toHaveBeenCalledWith(userId, activityType);
  });
  ...
  it('should filter user activities by date', async () => {
    ...
    jest.spyOn(service, 'filterUserActivitiesByDate').mockResolvedValue(mockActivities);

    const result = await controller.filterUserActivitiesByDate(userId, startDate, endDate);

    expect(result).toEqual(mockActivities);
    expect(service.filterUserActivitiesByDate).toHaveBeenCalledWith(userId, startDate, endDate);
  });
  ...
  it('should get recent user activities', async () => {
    ...
    const limit = 10;
    const mockActivities = [{ id: 1, userId, activityType: 'login', creationDate: new Date() }];
    jest.spyOn(service, 'getRecentUserActivities').mockResolvedValue(mockActivities);
  ...
  it('should delete old user activities', async () => {
    ...
    jest.spyOn(service, 'deleteOldUserActivities').mockResolvedValue({ count: 1 });

    await controller.deleteOldUserActivities(userId, cutoffDate);

    expect(service.deleteOldUserActivities).toHaveBeenCalledWith(userId, cutoffDate);
  });
  ...
  it('should throw ConflictException if create user activity fails', async () => {
    const createUserActivityDto = { userId: 1, activityType: 'login', creationDate: new Date() };
    jest.spyOn(service, 'createUserActivity').mockRejectedValue(new ConflictException());

    await expect(controller.createUserActivity(createUserActivityDto)).rejects.toThrow(ConflictException);
  });

  it('should update user activity', async () => {
    const activityId = 1;
    const patchUserActivityDto = { activityType: 'logout' };
    const mockActivity = { id: activityId, userId: 1, ...patchUserActivityDto, creationDate: new Date() };
  ...
  it('should throw BadRequestException if update user activity fails', async () => {
    ...
    await expect(controller.updateUserActivity(activityId, patchUserActivityDto)).rejects.toThrow(BadRequestException);
  });
